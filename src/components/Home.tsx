                    ...
                    <button
                        className={`absolute right-1.5 bottom-1.5 md:right-2 md:bottom-2 w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-200 shadow-lg play-btn-hover
                            ${isActive
                                ...
                                : 'bg-white/60 opacity-0 translate-y-1 md:group-hover:opacity-100 md:group-hover:translate-y-0 group-active:opacity-100 group-active:translate-y-0'
                            }`}
                        onClick={(e) => {
                            e.stopPropagation();
                            if (onPlay) onPlay();
            ...
            <button
                className={`w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-200 mr-1.5 md:mr-2 shadow-lg flex-shrink-0 play-btn-hover
                    ${isActive
                        ...
                        : 'bg-white/60 opacity-0 md:group-hover:opacity-100 group-active:opacity-100'
                    }`}
                onClick={(e) => {
                    e.stopPropagation();
                    if (onPlay) onPlay();
...
function ScrollSection({
    ...
    return (
        <section className="mb-6 relative group">
            ...
            <div className="flex overflow-x-auto pb-4 gap-4 snap-x snap-mandatory scrollbar-hide no-scrollbar md:grid md:grid-flow-col md:auto-cols-[160px] lg:auto-cols-[180px] xl:auto-cols-[200px] md:overflow-x-auto md:pb-4 md:scroll-smooth">
                {children}
            </div>
        </section>
    );
...
export function Home({ activePlaylistId, activeAlbumId, activeArtistId, onTrackSelect, currentTrack, isPlaying, setIsPlaying, playingContextId }: HomeProps) {
            ...
            const fetchDashboard = async () => {
                try {
                    ...
                    if (topTracksRes.status === 'fulfilled' && Array.isArray(topTracksRes.value)) {
                        setTopTracks(topTracksRes.value);
                    }
                    ...
                    if (recommendationsRes.status === 'fulfilled' && Array.isArray(recommendationsRes.value)) {
                        setRecommendations(recommendationsRes.value);
                    }
                    ...
                    if (likedSongsRes.status === 'fulfilled' && Array.isArray(likedSongsRes.value)) {
                        setLikedSongs(likedSongsRes.value);
                    }
    ...
    useEffect(() => {
        if ((activePlaylistId || activeAlbumId || activeArtistId) && token) {
            ...
            if (isLikedSongs) {
                fetchWithAuth(`${config.API_URL}/saved-tracks`)
                    .then(res => {
