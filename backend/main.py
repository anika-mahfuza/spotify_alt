...
def extract_audio_url(video_id: str) -> dict:
    ...
    logger.info(f"Extracting audio URL for: {video_id}")

    ydl_opts = YTDL_OPTS.copy()
    ydl_opts["playlist_items"] = "0"

    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        logger.info(f"Fetching video info from YouTube...")
        ...
        if not audio_formats:
            logger.warning(
                f"No pure audio formats for {video_id}, trying video+audio formats"
            )
            audio_formats = [
                f
                for f in formats
        ...
        best_audio = None
        for f in sorted(
            audio_formats, key=lambda x: x.get("tbr", 0) or 0, reverse=True
        ):
            if f.get("url"):
                best_audio = f
                logger.info(
                    f"Selected format: {f.get('format_id')} - {f.get('tbr')} kbps - {f.get('ext')}"
                )
                break

        if not best_audio and formats:
            logger.warning(f"No working formats found, trying all formats with URLs")
            for f in formats:
                if f.get("url") and f.get("acodec") != "none":
                    best_audio = f
                    logger.info(
                        f"Fallback format: {f.get('format_id')} - {f.get('ext')}"
                    )
                    break

        result = {
            "url": best_audio.get("url") if best_audio else info.get("url"),
        ...
        if result["url"]:
            cache.set(
                cache_key, json.dumps(result), ttl=300
...
async def stream(video_id: str):
    ...
    try:
        ...
        logger.info(f"Got stream URL for {video_id}: {url[:80]}...")

        # Determine content type from URL extension
        if url.endswith(".m4a") or ".m4a" in url:
            media_type = "audio/mp4"
        elif url.endswith(".webm") or ".webm" in url:
        ...
        async def iterfile():
            ...
            while True:
                ...
                if yielded_bytes > 0:
                    ...
                    logger.info(
                        f"Resuming stream for {video_id} from byte {yielded_bytes}"
                    )

                try:
                    # Create client with better YouTube compatibility
                    async with httpx.AsyncClient(
                        ...
                        async with client.stream("GET", current_url) as r:
                            ...
                            if r.status_code in (403, 410):
                                logger.warning(
                                    f"Stream {r.status_code} at {yielded_bytes} bytes for {video_id}"
                                )

                                if consecutive_errors >= max_consecutive_errors:
                                    logger.error(
                                        f"Max retries ({max_consecutive_errors}) reached for URL refresh - YouTube may be blocking this IP"
                                    )
                                    raise HTTPException(
                                        status_code=403,
                                        detail="YouTube stream blocked - try again later or use a different network",
                                    )
                    ...
                    logger.warning(
                        f"Stream interrupted at {yielded_bytes} bytes for {video_id}: {e}"
                    )
                    consecutive_errors += 1
...
def get_album_tracks(album_id: str, token: str):
    ...
    try:
        ...
        return {"name": album["name"], "images": album["images"], "tracks": tracks}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
        ...
        for term in search_terms:
            try:
                ...
                for item in items:
                    ...
                    if item and (
                        item.get("owner", {}).get("id") == "spotify"
                        or "Mix" in item.get("name", "")
                    ):
...
def get_recommendations(token: str):
    ...
    try:
        ...
        results = sp.recommendations(
            seed_tracks=seed_tracks[:2], seed_artists=seed_artists[:2], limit=20
        )
